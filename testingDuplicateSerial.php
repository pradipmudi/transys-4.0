...
$serialArray=array();
foreach($serialNoArray as $data)
{
	$serialArray[]=$data;
}
$count=count($serialArray);
$firstPos=$serialArray[0];
for($i=0;$i<$count;$i++)
{
	if($firstPos==$serialArray[$i])
	{
		$serialArray[$i]=$serialArray[$i]."^".$i;
		echo $serialArray[$i]."<br>";
	}
}
...
//{
//--------------------------------------	  
	  $query3 = "INSERT INTO serialno "."VALUES('$name2','$value','no','$price2','$dealer2','$entryDate','".$_SESSION['user']."','$categorySubCategory2','$vat2')";

	 $query4 = "INSERT INTO sellingprice "."VALUES('$name2','$value','no','$price2','$categorySubCategory2','$vat2')";	
		//execute query
